...
const existingUser = await userModel.findOne({email:req.body.email})
  ...
  if(!existingUser){
    ...
    req.files.map(e => {
        switch (e.fieldname) {
            case "image": user.image = e.filename
                break
        }
    })
    user.save()
...
router.post("/user/signup/validemail", async (req, res) => {
    ...
    const mailOptions = {
        ...
        text: "Welcome " + req.body.name + ', Your verification code is ' + veri_code,
...
router.post("/user/login", async (req, res) => {
    let user = await userModel.findOne({ email: req.body.email }).populate("role_id", "role")
    if (!user) {
...
router.post("/post/create", upload.any(), (req, res) => {
    ...
    req.files.map(e => {
        switch (e.fieldname) {
            case 'image': gallery.push(e.filename)
                break
        }
    })
    post.image = gallery 
...
router.post("/like", async (req, res) => {
    const existing = await likeModel.find({ post_id: req.body.post_id, likedBy: req.body.likedBy })
    if (!existing.length) {
        const like = await likeModel.create(req.body)
        res.send(like)
        const likedPost = await postModel.find({ _id: like.post_id })
        await postModel.updateOne({ _id: like.post_id }, { ...likedPost, likes: likedPost[0].likes + 1 })
    } else {
        const likedPost = await postModel.find({ _id: req.body.post_id })
        await likeModel.deleteOne({ post_id: req.body.post_id, likedBy: req.body.likedBy })
        await postModel.updateOne({ _id: req.body.post_id }, { ...likedPost, likes: likedPost[0].likes - 1 })
        res.send(false)
    }
})
...
router.get("/user/:id", async (req, res) => {
    const user = await userModel.findOne({ "_id": req.params.id }).populate("role_id", "role")
    res.send(user)
})

router.get("/post/:id", async (req, res) => {
    const post = await postModel.findOne({ "_id": req.params.id }).populate("post_category_id user_id", "name image post_category")
    res.send(post)
})
...
router.get("/comments", async (req, res) => {
    const comments = await commentModel.find().lean().populate("user_id post_id", "name title")
    res.send(comments)
})

router.get("/likes", async (req, res) => {
    const likes = await likeModel.find().lean().populate("likedBy post_id", "name title")
    res.send(likes)
})


router.get("/comments/:postId", async (req, res) => {
    const comments = await commentModel.find({ post_id: req.params.postId }).populate("user_id", "name")
    res.send(comments)
})

router.get("/:postId/likes", async (req, res) => {
    const likes = await likeModel.find({ post_id: req.params.postId }).populate("likedBy post_id", "name title")
    res.send(likes)
...
router.get("/:postId/like/:user_id", async (req, res) => {
    const like = await likeModel.findOne({ post_id: req.params.postId, likedBy: req.params.user_id }).populate("likedBy post_id", "name title")
    if (like) {
        res.send(true)
    } else {
        res.send(false)
...
router.put("/user", upload.any(), async (req, res) => {
    ...
    await userModel.updateOne({ _id: req.body.id }, body)
...
router.put("/post", upload.any(), async (req, res) => {
    ...
    if (post) {
        await postModel.updateOne({ _id: req.body.id }, body)
            .then(async () => res.send({ success: true, data: body }))
            .catch(err => {
                res.send(err)
            })
...
router.put("/comment", async (req, res) => {
    commentModel.updateOne({ _id: req.body.id }, req.body).then(async () => {
